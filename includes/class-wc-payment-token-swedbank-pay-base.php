...
class WC_Payment_Token_Swedbank_Pay_Base extends WC_Payment_Token_CC {
	/**
	 * Token Type String.
	 *
	 * @var string
	 */
	protected $type = 'Swedbank_Pay_Base';
	...
	public static function wc_get_account_saved_payment_methods_list_item( $item, $payment_token ) {
		if ( 'swedbank_pay' !== strtolower( $payment_token->get_type() ) && 'swedbank_pay_legacy' !== strtolower( $payment_token->get_type() ) && 'payex' !== strtolower( $payment_token->get_type() ) ) {
			return $item;
		}
	...
	public static function get_instance ($id) {
		$tokenType = WC_Payment_Tokens::get_token_type_by_id($id);
		$className = 'WC_Payment_Token_' . $tokenType;
		if (substr($tokenType, 0, 12) === 'Swedbank_Pay' && class_exists($className)) {
			try {
	...
	public static function wc_account_payment_methods_column_method( $method ) {
		$instance = WC_Payment_Token_Swedbank_Pay::get_instance($method['method']['id']);
		if ( $instance != null ) {
			echo $instance->get_display_name();
			return;
		}
